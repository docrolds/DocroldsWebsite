...
interface BusinessConsultation {
  tier: string;
  price: string;
  description: string;
  features: string[];
...
function Services(): JSX.Element {
  ...
  const mixingServices: MixingService[] = [
    {
      tier: 'Basic Mix',
      price: '$75',
  ...
  const businessConsultation: BusinessConsultation = {
    ...
    <section id="services" className="services-section">
      ...
      <div className="services-category">
        ...
        <div className="sessions-grid">
          {studioSessions.map((session: StudioSession, index: number) => (
      ...
      <div className="services-category">
        ...
        <div className="services-grid">
          {beatLeases.map((lease: BeatLease, index: number) => (
            <div key={index} className={`service-card ${lease.popular ? 'popular' : ''}`}>
              ...
              <ul className="service-features">
                {lease.features.map((feature: string, idx: number) => (
      ...
      <div className="services-category">
        ...
        <div className="services-grid four-col">
          {mixingServices.map((mix: MixingService, index: number) => (
            <div key={index} className={`service-card ${mix.popular ? 'popular' : ''}`}>
              ...
              <ul className="service-features">
                {mix.features.map((feature: string, idx: number) => (
                  <li key={idx}><i className="fas fa-check"></i> {feature}</li>
                ))}
              </ul>
              <button
        ...
        <div className="consultation-container">
          <div className="service-card consultation-card">
            ...
            <ul className="service-features two-column">
              {businessConsultation.features.map((feature: string, idx: number) => (
                <li key={idx}><i className="fas fa-check"></i> {feature}</li>
              ))}
            </ul>
            <button
