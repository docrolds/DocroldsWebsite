import { useEffect, useRef, JSX, MutableRefObject } from 'react';
...
function Instagram(): JSX.Element {
  ...
  const heartIntervalRef: MutableRefObject<ReturnType<typeof setInterval> | null> = useRef(null);
  ...
  useEffect(() => {
    ...
    const instagramObserver = new IntersectionObserver((entries: IntersectionObserverEntry[]) => {
  ...
  return (
    <section id="instagram" ref={sectionRef}>
      ...
      <div className={`instagram-posts-grid animate-on-scroll fade-up ${sectionVisible ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
        {posts.map((postId: string, index: number) => (
