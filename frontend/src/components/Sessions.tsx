...
interface Package {
  ...
  price: string;
  unit: string;
  key: 'starter' | 'professional' | 'premium' | 'business';
  recommended?: boolean;
}
...
function Sessions(): JSX.Element {
  ...
  const { ref: tableRef, isVisible: tableVisible } = useScrollAnimation<HTMLDivElement>({ threshold: 0.1 });
  ...
  // Session booking navigates to book page with pre-selected session type
  const getBookingUrl = (sessionType: string): string => {
    const sessionParam = encodeURIComponent(sessionType);
    return `/book?session=${sessionParam}`;
  };

  // Render feature cell value - clean and visible
  const renderFeatureValue = (value: boolean): JSX.Element => {
    return value ? (
