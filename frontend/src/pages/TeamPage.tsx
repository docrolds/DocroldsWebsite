...
function TeamPage(): ReactNode {
  const [teamMembers, setTeamMembers] = useState<TeamMember[]>([]);
  const [expandedBios, setExpandedBios] = useState<Set<string | number>>(new Set());
  ...
  const toggleBio = (id: string | number): void => {
    setExpandedBios((prev: Set<string | number>) => {
        ...
        {teamMembers.length > 0 ? (
          <div className="team-grid">
            {teamMembers.map((member: TeamMember) => {
              // Parse roles
              const roles = parseJsonField<string>(member.role);
              const roleDisplay = roles.map((r: string) => r.charAt(0).toUpperCase() + r.slice(1)).join(' / ');

              // Parse credits
              const credits = parseJsonField<string>(member.credits);

              // Parse placements
              const placements = parseJsonField<string>(member.placements);
